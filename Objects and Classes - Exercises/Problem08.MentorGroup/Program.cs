...
class Program
{
    static void Main()
    {
        ...
        while (command != "end")
        {
            ...
            if (!userNames.ContainsKey(name)) userNames.Add(name, new List<DateTime>());
        ...
        while (command != "end of comments")
        {
            string[] input = Console.ReadLine().Split(new char[] { '-' }, StringSplitOptions.RemoveEmptyEntries).ToArray();
            string name = input[0];
            if (name == "end of comments") break;
            string comment = input[1];
            List<string> comments = new List<string>();
            comments.Add(comment);
            if (userNames.ContainsKey(name) && !userComments.ContainsKey(name)) userComments.Add(name, new List<string>());
            if (userComments.ContainsKey(name)) userComments[name].AddRange(comments);
        }
